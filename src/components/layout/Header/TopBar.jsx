    ...
    <div className={`${theme.colors.primaryBg} relative overflow-hidden text-gray-200 text-xs md:text-sm px-4 md:px-8 py-2 flex flex-col md:flex-row md:items-center md:justify-between gap-3 ${theme.shadows.md} z-50 border-b ${theme.borders.accentSubtle}`}>
      ...
      <div className="flex items-center gap-4 relative z-10">
        ...
        <div className="leading-tight flex flex-col justify-center">
          ...
          <h1 className={`text-xs md:text-sm font-semibold ${theme.colors.accentText} tracking-wider`}>
      ...
      <div className="flex flex-wrap gap-4 md:gap-6 justify-start md:justify-end relative z-10">
        {links.map((link) => (
          ...
          >
            {link.label}
            {/* Animated underline */}
            <span className={`absolute left-0 -bottom-1 w-0 h-0.5 ${theme.colors.accentBg} transition-all duration-300 group-hover:w-full`}></span>
          </Link>
        ))}
      </div>
    </div>
